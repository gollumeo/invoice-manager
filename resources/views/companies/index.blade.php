...
<main class="w-4/5 md:w-2/3 self-center mb-8 flex-1">
    ...
    <div class="overflow-x-auto">
        <table class="w-full divide-y-2 divide-slate-300 min-[1300px]:table-fixed">
            <thead>
            <tr class="divide-slate-300 text-left bg-gray-50">
                <th class="w-8 p-2">#</th>
            ...
            @foreach($companies as $company)
                <tr class="divide-y-2 divide-slate-300 hover:bg-gray-100">
                    <td class="text-left p-2">{{ $company->id }}</td>
                    <td class="min-w-content p-2 text-red-700 font-medium break-word min-[1300px]:break-normal">
                        <a href="/companies/{{$company->id}}">{{ $company->name }}</a>
                    </td>
                    <td class="w-1/6 p-2 break-word min-[1300px]:break-normal">{{ $company->address }}</td>
                    <td class="p-2">{{ $company->zipcode }}</td>
                    <td class="p-2">{{ $company->country }}</td>
                    <td class="p-2 break-word min-[1300px]:break-normal">{{ $company->vat }}</td>
