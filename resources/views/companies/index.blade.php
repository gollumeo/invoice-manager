...
<main class="w-4/5 md:w-2/3 self-center mb-8">
    ...
    <form action="">
        <input type="search" name="companies" id="" placeholder="Search for a company here..." class="px-4 py-2 w-1/4 placeholder:italic rounded-2xl">
    </form>
        ...
        @foreach($companies as $company)
            <tr class="divide-y-2 divide-slate-300">
                ...
                <td class="text-justify w-2 text-rose-700 p-2">
                    <form action="/companies/edit/{{ $company->id }}" method="POST" class="inline" title="Update this company" alt="update">
                        @csrf
                        <button><i
                                class="fa-solid fa-pen-to-square fa-xl mr-14 hover:scale-125 transition hover:cursor-pointer"></i>
                        </button>
                    </form>
                    <form action="/companies/delete/{{ $company->id }}" method="POST" class="inline" title="Delete this company" alt="delete">
                        @csrf
                        @method('DELETE')
                        <button><i
